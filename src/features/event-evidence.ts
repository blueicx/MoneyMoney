...
export function matchesEventScope(item: { scope: string; instrumentId?: string | null }, scope: string, instrumentId?: string): boolean {
  ...
  // A selected instrument timeline is exact. Market-wide events remain
  // available from the market workspace, but must not be copied into every
  // selected stock, option, crypto asset, or prediction market.
  if (instrumentId) return item.instrumentId === instrumentId;
  return true;
}
