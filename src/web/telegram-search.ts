...
export function buildTelegramStockSearchRows(items: TelegramStockSearchItem[]): Array<Array<{ text: string; callback_data: string }>> {
  return items.map((item) => {
    const label = String(item.zhName || item.name || item.code).slice(0, 8);
    return [
      { text: `加入自选 ${label}`, callback_data: `watch:add:${item.code}` },
