...
test('stock dashboard requests share the market cancellation boundary', () => {
  ...
  assert.match(html, /async function loadStockQuotes\(\)[\s\S]*const controller = new AbortController\(\)[\s\S]*currentStockDashboardController = controller[\s\S]*Promise\.allSettled\(\[loadStockIndices\(signal\)\]\)/);
